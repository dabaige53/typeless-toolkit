...
const DEFAULT_CONFIG = {
  ...
  paywall: {
    // v1.8.0 的目标文件路径(asar 内相对路径数组)
    file_path: ['dist', 'renderer', 'static', 'js', '2J71HyJ6.mjs'],
    // 等长 13 字节替换:gn(x) -> (0,x),让付费墙渲染函数直接返回入参即跳过显示
...
// ---------- Typeless.exe 探测 ----------
// 优先级: config.typeless_exe → 环境变量 TYPELESS_EXE → %LOCALAPPDATA%\Programs\Typeless\Typeless.exe → 抛错
function detectTypelessExe() {
  ...
  const def = path.join(localApp, 'Programs', 'Typeless', 'Typeless.exe');
  const p = tryPath(def);
  if (p) return def;
  throw new Error(
    '未找到 Typeless.exe。请在 config.json 里配置 typeless_exe 路径(指向你的 Typeless 安装目录下的 Typeless.exe)。' +
    '默认探测路径:' + def
  );
}
...
const USERDATA_DIR = path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'Typeless.exe');
const DEVICE_CACHE_DIR = path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'Typeless', 'Cache');
const CRED_TARGET = 'Typeless.deviceIdentifier';
const ASAR_PATH = TYPELESS_EXE ? path.join(path.dirname(TYPELESS_EXE), 'resources', 'app.asar') : '';
...
function killTypeless() {
  try { execSync('taskkill /F /IM Typeless.exe', { stdio: 'ignore' }); } catch (e) {}
}
function launchTypeless() {
  if (!TYPELESS_EXE) throw new Error('Typeless.exe 路径未配置,无法启动');
  spawn(TYPELESS_EXE, [`--remote-debugging-port=${CDP_PORT}`], { detached: true, stdio: 'ignore' }).unref();
}

// ---------- 解除设备限制 ----------
async function resetDevice() {
  killTypeless(); await sleep(1500);
  // 1) 删 Credential Manager 里的设备 ID(真正来源,对应 Mac Keychain)
  try { execSync(`cmdkey /delete:${CRED_TARGET}`, { stdio: 'ignore' }); } catch (e) {}
  // 2) 删 device.cache
...
// (process.noAsar 对 fs.readFileSync 在新版 Electron 不生效,改用 cmd copy + 非 .asar 后缀中转)
function asarToTmp() {
  const tmp = path.join(os.tmpdir(), `tt_asar_${process.pid}_${Date.now()}.bin`);
  execSync(`cmd /c chcp 65001 >nul & copy /b /y "${ASAR_PATH}" "${tmp}"`, { stdio: 'ignore' });
  return tmp;
}
// 把临时文件覆盖回 app.asar(同样用 cmd copy 绕 hook)
function tmpToAsar(tmp) {
  execSync(`cmd /c chcp 65001 >nul & copy /b /y "${tmp}" "${ASAR_PATH}"`, { stdio: 'ignore' });
}
...
function patchPaywall() {
  ...
  // 首次备份:asar 用 cmd copy(绕 hook),exe 用 fs
  if (!fs.existsSync(asarBak)) { try { execSync(`cmd /c chcp 65001 >nul & copy /b /y "${ASAR_PATH}" "${asarBak}"`, { stdio: 'ignore' }); } catch (e) {} }
  ...
  try {
    ...
    // 6) 改 Typeless.exe 内嵌整头 SHA256(exe 非 asar,直接 fs 操作)
    const exfd = fs.openSync(TYPELESS_EXE, 'r+');
    const estat = fs.statSync(TYPELESS_EXE);
    ...
    if (ecnt < 1) throw new Error('exe 里找不到旧整头 hash(可能已被改过或版本不符),已还原请检查');
    ...
    return {
      already: false, done: true, exe_hits: ecnt,
      file_hash: { old: oldHash, new: newHash },
      header_hash: { old: oldHeaderHash, new: newHeaderHash },
      msg: '补丁已打好,升级/会员弹窗将不再弹出(重启 Typeless 生效)',
    };
  } catch (e) {
    // 失败:exe 可能已改,从备份还原;asar 未覆盖回(只改了 tmp),保持原样无需还原
    try { if (fs.existsSync(exeBak)) fs.copyFileSync(exeBak, TYPELESS_EXE); } catch (_) {}
    throw e;
