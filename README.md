...
纯 Node.js + 单页前端,无外部依赖,支持 Windows 与 macOS。
...
- **去弹窗 = 两层 asar 完整性补丁**:Electron 有两道 asar 完整性校验——asar 头里每个文件的
  per-file SHA256,以及可执行文件内嵌的整头 SHA256。改 asar 内容必须同时更新这两层,否则启动闪退。
  本工具用等长字节替换(如 `_n(x)` → `(0,x)`,旧版曾为 `gn(x)`)把付费墙渲染调用变成无操作表达式;
  版本变了 minified 名会变,需按「弹窗补丁适配」重定位。
...
Typeless 更新后,`.mjs` 文件名和 minified 变量名(`_n`、`gn`、`_0x…` 等)可能改变。
